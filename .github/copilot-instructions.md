...
**Local graph store is pluggable (2026-09-19).** `HubStoreInterface` (Graph/) has two implementations: `SqlHubStore` (default — tables `bibframehub_hub/agent/relation` in VuFind's own MariaDB, loaded from the Hugging Face Parquet export via `php public/index.php bibframehub/load-hubs --dir <tsv>`; ~5 min) and the legacy `Neo4jService`. `HubStoreFactory` picks by `[HubStore] backend = sql|neo4j|none`. `BibframeHub.php` still names the property `$neo4j` but it is typed to the interface. The Docker demo runs Neo4j-free; the Neo4j containers described below no longer exist on this machine. Data pipeline: `tools/hf-dataset/ttl_to_parquet.py` (LC TTL → Parquet, ~1 min) → HF dataset `jimfhahn/lc-bibframe-hubs` → `export_sql_tsv.py` → loader. UUID key columns are `ascii_bin`; keep it that way (MariaDB 11's default uca1400 collation broke joins once). Sidebar links to the hosted Observable explorer (HF Space `jimfhahn/bibframe-hub-explorer`, `notebooks/`) via `[Display] explorerUrl`.
